...
const ConnectButton: FC = () => {
  ...
  useEffect(() => {
    if (publicKey?.toBase58() !== "" && publicKey?.toBase58() !== undefined) {
      if (swapState) {
        getTokenListMoreThanZero(publicKey.toBase58(), setTokenList, setLoadingState, forceRefreshTokens);
      } else {
        getTokenListZeroAmount(publicKey.toBase58(), setTokenList, setLoadingState, forceRefreshTokens);
      }
    }
