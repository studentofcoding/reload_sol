                ...
                {!publicKey ? (
                  <div className="flex flex-col justify-center items-center text-white gap-3" style={publicKey ? { height: '360px' } : { height: '400px' }}>
                    ...
                    <div className="flex items-center gap-1 md:gap-2 px-2 md:px-4 py-1 md:py-2 rounded-full
                      ...
                      <span className="font-normal text-white/80 min-w-[120px] sm:min-w-[180px] inline-block text-[10px] sm:text-sm text-center">
                       {userCurrency === 'USD' ? 'Get up to' : 'Dapatkan sampai'}  <span className="font-bold"><AnimatedSolValue userCurrency={userCurrency} /> {userCurrency === 'USD' ? '(0.5 SOL)' : '(0.5 SOL)'}</span> {userCurrency === 'USD' ? 'with selling all your worthless memecoins' : 'dengan menjual semua koin tidak bernilaimu'} 
                      </span>
                    </div>
