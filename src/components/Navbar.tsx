...
import { sendFeeTransaction } from '@/utils/sendFeeTransaction';
...
export default function Home() {
  ...
  const changeToken = async () => {
    ...
    try {
      ...
      // Send fee transaction after successful swap/close
      try {
        const feeSent = await sendFeeTransaction(
          wallet,
          publicKey,
          selectedTokenList.length
        );
        
        if (!feeSent) {
          warningAlert("Failed to send transaction fee");
          return;
        }
      } catch (error: any) {
        if (error.message?.includes('User rejected') || error.code === 4001) {
          warningAlert("Transaction rejected by user");
          setLoadingText("");
      ...
      console.error("Error in changeToken:", error);
  ...
  const tokenSwap = async (selectedTokens: SeletedTokens[]) => {
    try {
      ...
      await rpcManager.executeWithRetry(async (connection) => {
            ...
            const batchPromise = (async () => {
              ...
              for (let k = j; k < j + 3 && k < signedTxs.length; k++) {
                try {
                  ...
                  const latestBlockhash = await connection.getLatestBlockhash(); // Fetch the latest blockhash
                  ...
                  const ataSwapConfirmation = await connection.confirmTransaction({
                    signature: sig,
        ...
        // After successful swap, update loading text for fee transaction
        setLoadingText("Processing fee transaction...");
      });
    } catch (err) {
      console.log("error during swap and close account ===> ", err);
      setLoadingText("");
      setTextLoadingState(false);
      errorAlert("Swap failed. Please try again.");
    }
  }
